...
import shoesAsbestos from '../fixtures/shoes-asbestos.json'
...
test("Reading the strip offsets for the shoes", () => {
    ...
    const file = readFileSync(`./bin/PL00P003.BIN`);
    ...
    const names = ["20_ASBESTOS_RIGHT_FOOT", "21_ASBESTOS_LEFT_FOOT"];
    ...
    expect(meshes).toEqual(shoesAsbestos);
});

test('Re-encoding the vertices read from the shoes(asbestos)', () => {
    const file = readFileSync(`./bin/PL00P003.BIN`);
    const dat = file.subarray(0x30, 0x30 + 0x2b40);
    const { buffer } = Buffer.from(dat);
    const reader = new ByteReader(buffer as ArrayBuffer);

    shoesAsbestos.map((mesh) => {
...
test('Re-encoding the tris read from the shoes(asbestos)', () => {
    const file = readFileSync(`./bin/PL00P003.BIN`);
    const dat = file.subarray(0x30, 0x30 + 0x2b40);
    const { buffer } = Buffer.from(dat);
    const reader = new ByteReader(buffer as ArrayBuffer);
...
test.skip('Re-encoding the quad read from the shoes(asbestos)', () => {
    const file = readFileSync(`./bin/PL00P003.BIN`);
    const dat = file.subarray(0x30, 0x30 + 0x2b40);
    const { buffer } = Buffer.from(dat);
    const reader = new ByteReader(buffer as ArrayBuffer);

...
import rightArm from "../fixtures/right-arm.json";
...
test("Reading the strip offsets for the right arm", () => {
  ...
  const names = ["50_RIGHT_SHOULDER", "51_RIGHT_ARM", "52_RIGHT_HAND"];
...
test("Re-encoding the tri read from the right arm", () => {
  ...
  rightArm.map((mesh) => {
    ...
    for (let i = 0; i < triCount; i++) {
      ...
      expect(
        Math.floor((a.u - PIXEL_ADJUSTMEST) / PIXEL_TO_FLOAT_RATIO),
      ).toEqual(au);
      expect(
        Math.floor((a.v - PIXEL_ADJUSTMEST) / PIXEL_TO_FLOAT_RATIO),
      ).toEqual(av);
      expect(
        Math.floor((b.u - PIXEL_ADJUSTMEST) / PIXEL_TO_FLOAT_RATIO),
      ).toEqual(bu);
      expect(
        Math.floor((b.v - PIXEL_ADJUSTMEST) / PIXEL_TO_FLOAT_RATIO),
      ).toEqual(bv);
      expect(
        Math.floor((c.u - PIXEL_ADJUSTMEST) / PIXEL_TO_FLOAT_RATIO),
      ).toEqual(cu);
      expect(
        Math.floor((c.v - PIXEL_ADJUSTMEST) / PIXEL_TO_FLOAT_RATIO),
      ).toEqual(cv);
      expect(
        indexA | (indexB << 7) | (indexC << 14) | (materialIndex << 28),
      ).toEqual(dword);
    }
  });
});
...
test("Re-encoding the quad read from the right arm", () => {
  ...
  rightArm.map((mesh) => {
    ...
    for (let i = 0; i < quadCount; i++) {
      ...
      expect(
        Math.floor((d.u - PIXEL_ADJUSTMEST) / PIXEL_TO_FLOAT_RATIO),
      ).toEqual(du);
      expect(
        Math.floor((d.v - PIXEL_ADJUSTMEST) / PIXEL_TO_FLOAT_RATIO),
      ).toEqual(dv);

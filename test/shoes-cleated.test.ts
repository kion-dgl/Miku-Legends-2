...
test('Re-encoding the vertices read from the shoes(cleated)', () => {
    const file = readFileSync(`./bin/PL00P002.BIN`);
    const dat = file.subarray(0x30, 0x30 + 0x2b40);
    const { buffer } = Buffer.from(dat);
    const reader = new ByteReader(buffer as ArrayBuffer);

    shoesCleated.map((mesh) => {
...
test('Re-encoding the tris read from the shoes(cleated)', () => {
    const file = readFileSync(`./bin/PL00P002.BIN`);
    const dat = file.subarray(0x30, 0x30 + 0x2b40);
    const { buffer } = Buffer.from(dat);
    const reader = new ByteReader(buffer as ArrayBuffer);
...
test('Re-encoding the quad read from the shoes(cleated)', () => {
    const file = readFileSync(`./bin/PL00P002.BIN`);
    const dat = file.subarray(0x30, 0x30 + 0x2b40);
    const { buffer } = Buffer.from(dat);
    const reader = new ByteReader(buffer as ArrayBuffer);

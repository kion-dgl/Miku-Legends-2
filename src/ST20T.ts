...
const updatePoster = (bin: Buffer, pngPath: string) => {
  ...
  for (let i = 0; i < 16; i++) {
    ...
    includedPal.writeUInt16LE(red, i * 2);
  }
  ...
  if (ofs <= 0x3f800) {
    ...
    throw new Error("reaverbot painting too short");
  } else if (len > 0x40000) {
    console.log("too long");
    throw new Error("reaverbot painting too long");
  } else {
    console.log("yaya!!!");
  }

  console.log("End: 0x%s", ofs.toString(16));
  bin.writeInt16LE(bodyBitField.length, 0x3c024);
};

const updateCarlbania = (poster: string) => {
  const bin = readFileSync("bin/carlbania-ST20T.BIN");
  updatePoster(bin, poster);
  writeFileSync("out/carlbania-ST20T.BIN", bin);
};

export default updateCarlbania;
export { updateCarlbania };

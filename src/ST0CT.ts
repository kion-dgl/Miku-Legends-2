...
const updateTigerRoom = (bin: Buffer, pngPath: string) => {
  const pngData = readFileSync(pngPath);

  const imgOfs = 0x4800;
  // 38295 (light brown)
  const pal: number[] = [
    50773, 34239, 34076, 36479, 60318, 40170, 43774, 48793, 34038, 37384, 40148,
    48626, 42556, 38295, 44398, 56157,
  ];
  ...
  if (ofs <= 0x7800) {
    ...
    throw new Error("tiger painting too short");
  } else if (len > 0x8000) {
    console.log("too long");
    throw new Error("tiger painting too long");
  } else {
    console.log("yaya!!!");
  }

  bin.writeInt16LE(bodyBitField.length, 0x4824);
};

const updateYosyonkePaintings3 = (tiger: string) => {
  const bin = readFileSync("bin/yosyonke-ST0CT.BIN");
  updateTigerRoom(bin, tiger);
  writeFileSync("out/yosyonke-ST0CT.BIN", bin);
};

export default updateYosyonkePaintings3;
export { updateYosyonkePaintings3 };
